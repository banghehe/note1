;(function ($, window, document, undefined) {
    ...
    var plugin_url  = 'http://localhost/notepad-html/';
    ...
    $(function() {
        ...
        $('.wls-type').on('click', function () {
            ...
            if ($state === 'all') {
                ...
                $img.attr('src', '/assets/images/worksheet/Search_Type_PRACTICE.png');
            } else if ($state === '1') {
                $this.attr('data-state', '2');
                $img.attr('src','/assets/images/worksheet/Search_Type_TEST.png');
            } else if ($state === '2') {
                $this.attr('data-state', 'all');
                $img.attr('src','/assets/images/worksheet/Search_Type_ALL.png');
            }
        });
...
// NEW_EVENT________________________
var add_friend = 0;
$(".add_friend").click(function() {
    add_friend++;
    if ($(".popup_user_menu").hasClass('hidden')) {
        $(".popup_user_menu").removeClass("hidden");
        $(".popup_user_menu").addClass('active');

    } else {
        $(".popup_user_menu").addClass("hidden");
        $(".popup_user_menu").removeClass('active');

    }
});

var add_friend = 0;
$(".invite_user_list").click(function() {
    add_friend++;
    if ($(".popup_user_list").hasClass('hidden')) {
        $(".popup_user_list").removeClass("hidden");
        $(this).addClass('active');
    } else {
        $(".popup_user_list").addClass("hidden");
        $(this).removeClass('active');
    }
});

$(".roll_back_footer_list").click(function() {
    $(".popup_user_list").addClass("hidden");            
});

$(".exit_messages").click(function() {
        $(".popup_messages").removeClass("active");
        $(".popup_messages").addClass('hidden');
        $(".popup_messages_3").removeClass("active");
        $(".popup_messages_3").addClass('hidden');
        $(".popup_user_menu").css("bottom",'37px');
});


$(".invite_user").click(function(){
    if ($(".input_user_invite").hasClass('hidden')) {
        $(".input_user_invite").removeClass("hidden");
        $(".invite_user_list").addClass("hidden");
        $(".invite_user").addClass("hidden");
    } else {
        $(".input_user_invite").addClass("hidden");
        $(".invite_user_list").addClass("active");
        $(".invite_user_list").removeClass("hidden");

    }
});

$(".img_toggle").click(function(){
    $(".click_invite").removeClass("hidden");
    $(".invite_user_list").removeClass("hidden");
    $(".input_user_invite").removeClass("active");
    $(".input_user_invite").addClass("hidden");
    $(".invite_user").removeClass("hidden");
    $(".invite_user").addClass("active");
});
...
$(".delete_contact").click(function() {
    if ($(".popup_messages_2").hasClass('hidden')) {
        $(".popup_messages_2").removeClass("hidden");
        $(".popup_messages_2").addClass('active');
        $(".popup_user_list").css("bottom",'100px');
        $(".popup_user_menu").css("bottom",'100px');
        $(".roll_back_footer_list img").css("bottom",'125px');
        $(".user_list_center button").css("bottom",'115px');
        $(".delete_contact").css("bottom",'128px');
        $(".delete_contact img").css("bottom",'130px');
    }
});


$("#exit_messages_2").click(function() {
    if ($(".popup_messages_2").hasClass('active')) {
        $(".popup_messages_2").removeClass("active");
        $(".popup_messages_2").addClass('hidden');
        $(".popup_user_list").css("bottom",'37px');
        $(".popup_user_menu").css("bottom",'37px');
        $(".roll_back_footer_list img").css("bottom",'55px');
        $(".user_list_center button").css("bottom",'45px');
        $(".delete_contact").css("bottom",'57px');
        $(".delete_contact img").css("bottom",'60px');
    }
});
...
$('#divrubber').draggable();
        ...
        $('#click_delete_friend_list').click(function(){
            var checkbox = document.getElementsByName('user');
                
                var data_value = '';
                for (var i = 0; i < checkbox.length; i++){
                    if (checkbox[i].checked === true){
                        data_value = data_value +' '+ checkbox[i].value;  
                ...
                $.ajax({
                        ...
                        data:'string='+ data_value  + '&action=iii_notepad_delete_friend',
                        dataType: 'json',
                        beforeSend: function () {
    ...
    $('#click_add_friend_list').click(function(){
        var name = $('.display_name').text();
        var email_fr = $('#input_email').val();
        var name_fr = `<?php echo get_user_by('email', $value)->display_name?>`;  
            if(name !== ''){
               $.ajax({
                type: 'POST',
                url: iii_script.ajax_url,
                data: $('#form_check').serialize() + '&action=iii_notepad_add_friend',
                dataType: 'json',
                beforeSend: function () {
                ...
                success: function (response) {
                    if (response === '1') {
                        $(".popup_messages_3").removeClass("hidden");
                        $(".popup_messages_3").addClass('active');
                        $(".popup_messages").addClass('hidden');
                        $(".popup_user_menu").css("bottom",'100px');
                        $('.error_messages').empty();
                        $('.error_messages').append('<span>Already exists in your friends list</span>');
                    }else if(response === '2'){
                        $(".popup_messages_3").removeClass("hidden");
                        $(".popup_messages_3").addClass('active');
                        $(".popup_messages").addClass('hidden');
                        $(".popup_user_menu").css("bottom",'100px');
                        $('.error_messages').empty();
                        $('.error_messages').append('<span>Invalid address email</span>');
                        ...
                        $(".popup_user_menu").css("bottom",'100px');
                        $('.error_messages').empty();
                        $('.error_messages').append('<span>Email entered is not valid</span>');
                    }else{
                        $('#friend-list').append(`<li><input type="checkbox" class="radio_buttons option-input-3 radio" name="user" value="${email_fr}" /><span>${response} (${email_fr})</span></li>`);
                        $(".popup_messages_3").removeClass("hidden");
                        $(".popup_messages_3").addClass('active');
                        $(".popup_messages").addClass('hidden');
                        $(".popup_user_menu").css("bottom",'100px');
                        $('.error_messages').empty();
                        $('.error_messages').append('<span>Added to your friends list successfully.</span>');
                    }
                }
            });
        ...
        $('#click_check_email').on('click', function () {

            $.ajax({
                type: 'POST',
                url: iii_script.ajax_url,
                data: $('#form_check').serialize() + '&action=iii_notepad_worksheet_check_mail',
                dataType: 'json',
                beforeSend: function () {
                ...
                success: function (response) {
                    if (response === 'Available') {
                        $(".popup_messages").removeClass("hidden");
                        $(".popup_messages").addClass('active');
                        $(".popup_user_menu").css("bottom",'100px');
                        $(".popup_messages_3").addClass('hidden');     
                        document.getElementById('insert_email').innerHTML = $("#input_email").val();
                        ...
                        $(".popup_user_menu").css("bottom",'100px');
                        $('.error_messages').empty();
                        $('.error_messages').append('<span>Invalid User. Please try again.</span>');
                    }
                }
            });
        });
        ...
        $(document).on('click', '.icon-selector', function() {
            ...
            if (!$(this).hasClass('active') && !$(this).hasClass('none-visiable')) {
                ...
                if ($('.icon-layer.btn-grid').hasClass('active')) {
                    ...
                    if (grid) {
                        $('#panel').css('background-image', 'url(/assets/images/notepad/grid/grid-' + grid + '.png)')
                    }
                }
            } else if ($(this).hasClass('active')) {
                $(this).removeClass('active').addClass('none-visiable');
                $(this).find('img').attr('src', '/assets/images/notepad/layer/Layer_' + (cnt - 3) + '_Not_Visible.png');
                ...
                $("#layers-body li").removeClass('active');
                $(this).find('img').attr('src','assets/images/notepad/layer/layer-' + (cnt - 3) + '.png');
...
function logMessage(alias, text, me, testichat, emoji) {
    ...
    $(window).on("load", function(){
        ...
        $(".user_list_detail").mCustomScrollbar({
           theme:"3d-dark",
